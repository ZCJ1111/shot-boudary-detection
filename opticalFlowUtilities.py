...
class generateOpticalFlowResults():
    ...
    def __init__(self,filename, start_frame=0, end_frame=None, output_path = 'results_of_MR', hist_eq = False, seek_forward = 1):
        ...
        print('max_frame: '+str(self.max_frame))
    ...
    def runOpticalFlow(self):
        ...
        Parallel(n_jobs=-1)(delayed(self.performOpticalFlowAndSave)(i, self.hist_eq) for i in tqdm(range(self.start_frame,self.end_frame-self.seek_forward)))
        
    def performOpticalFlowAndSave(self, frame, hist_eq=False):
    
        if not os.path.exists(self.output_path+'/frame_'+self.mid+str(frame)+'.png') or not os.path.exists(self.output_path+'/optical_flow_s'+str(self.seek_forward)+'_'+str(frame)+'.png') or not os.path.exists(self.output_path+'/np_flow_s'+str(self.seek_forward)+'_'+str(frame)+'.npy'):

            vid0 = imageio.get_reader(self.filename,  'ffmpeg') # hack around a bug in the implementation
            vid1 = imageio.get_reader(self.filename,  'ffmpeg')

            prev_frame_gray = cv.cvtColor(vid0.get_data(frame), cv.COLOR_BGR2GRAY)
            new_frame_gray = cv.cvtColor(vid1.get_data(frame+self.seek_forward), cv.COLOR_BGR2GRAY)
            ...
            np.save(self.output_path+'/np_flow_s'+str(self.seek_forward)+'_'+str(frame)+'.npy',flow)
            cv.imwrite(self.output_path+'/optical_flow_s'+str(self.seek_forward)+'_'+str(frame)+'.png',self.dispOpticalFlow(prev_frame_gray,flow,5,'test')*255)
            cv.imwrite(self.output_path+'/frame_'+self.mid+str(frame)+'.png',prev_frame_gray)
        else:
            pass
