...
class detectFades():
    ...
    def __init__(self, filename, output_path0='results_of_MR', rgb=False, seek_forward = 1, seek_forward_cnn = 5):
        ...
        #self.seek_forward_cnn = seek_forward_cnn
        
        #if not os.path.exists(self.output_path0+'/jsons'):
        #    os.mkdir(self.output_path0+'/jsons')
            
            
    def calculateHistogramOverlapSingle(self, frame_img, frame_of): #frame):
    
        if self.rgb:
            intersect = float(self.histogram_intersection_rgb(frame_img, frame_of))
        else:
            prev_img = cv.cvtColor(frame_img, cv.COLOR_BGR2GRAY)
            next_img = cv.cvtColor(frame_of, cv.COLOR_BGR2GRAY)
