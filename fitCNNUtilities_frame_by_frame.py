...
class analysisWithCNN():
    ...
    def __init__(self, output_path0_model, hist_eq0, rgb=False, seek_forward = 1):
    ...
    def runModelOnTestData(self, filename_test, frame): #output_path0_test, output_path0_ref,  #end_frame, start_frame = 0):
        ...
        self.createTestDataLoader(frame)
        ...
        kk10 = 0
        for sample, target in self.dataloader_test:
            ...
            output_test = model(x_test).cpu().detach().numpy()
        ...
        return logistic.cdf(output_test)
  
    def createTestDataLoader(self, frame_cnn):
        ...
        targets_test = np.zeros(len([1]))
        dataset_test = self.MyDataset([frame_cnn], targets_test, transform=self.transform)
